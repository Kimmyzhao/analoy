...
import cn.epalmpay.analoy.controller.TestController;
...
public class ZhongHuiController {
  ...
  /**
   * 
   * { "respTime": "20150516163252", "isSuccess": true, "respCode": "SUCCESS", "respMsg": "登录成功",
   * "status": "2222", "name": "刘引惟", "cardTail": "9477", "ksnNo": "501010033373", "bluetoothName":
   * "", "serialType": "0.78", "model": "zfmini", "nextReqNo": 67, "argument": {}, "sessionKey":
   * "01B6A60F77F58D5AA74182DB826198B4", "keyCheck": "5C868F01", "isBluetooth": false }
   * 
   * @param reqTime
   * @param loginName
   * @param password
  ...
  public String login(String reqTime, String loginName, String password, String position,
      String appVersion, String product) {

    return TestController.loginResult;
  }

  @RequestMapping(value = "getAgentInfoByEqno")
  public String getAgentInfoByEqno(String loginName, String password, String position,
      String appVersion, String product) {
    ...
    if (loginName == null || "".equals(loginName)) {
      return StringUtils.parseObjectToJSONString(new Resp(respTime, false,
          Constant.ILLEGAL_ARGUMENT, "缺少参数loginName"));
    } else if (password == null || "".equals(password)) {
      return StringUtils.parseObjectToJSONString(new Resp(respTime, false,
          Constant.ILLEGAL_ARGUMENT, "缺少参数password"));
    } else if (!list.contains(loginName) || !"123456".equals(password)) {
      return StringUtils.parseObjectToJSONString(new Resp(respTime, false,
          Constant.ILLEGAL_LOGIN_OR_PASSWD, "登录名或密码错误"));
    }
  ...
  public String activate(String reqTime, String licenseCode, String ksnNo, String appVersion,
      String product) {
    String response = "";
    String date = StringUtils.dateToString(new Date(), "yyyyMMddHHmmss");
    if (licenseCode == null || "".equals(licenseCode)) {
      response = StringUtils.parseObjectToJSONString(new Resp(date, false,
          Constant.ILLEGAL_ARGUMENT, "缺少参数licenseCode"));
    ...
    if (eq == null) {
      response = StringUtils.parseObjectToJSONString(new Resp(date, false,
          Constant.ACTIVATED_ERROR_CODE, "设备不存在或者激活码错误"));
      logger.info(response);
      return response;
    } else {
      if (eq.getActivated() == EquipMent.ACTIVATE_STATUS_NO_ACTIVED) {// 未激活
        equimentService.activatedByKsnNo(ksnNo, EquipMent.EQTYPE_ZHONGHUI,
            EquipMent.ACTIVATE_STATUS_NO_REGISTED);// 更改为激活未注册
  ...
  public String register(String reqTime, String ksnNo, String name, String mobile, String password,
      String registPosition, String appVersion, String product) {
   ...
   * 模糊查询银行
   * 
   * { "respTime": "20150520130443", "isSuccess": true, "respCode": "SUCCESS", "respMsg": "成功",
   * "total": 647, "tip": "结果较多, 建议使用精确关键字, 例如添加地名等", "banks": [ { "bankDeposit":
   * "中国工商银行股份有限公司无锡吴桥支行", "unionBankNo": "102302002105" }, { "bankDeposit": "中国工商银行股份有限公司无锡河埒支行",
   * "unionBankNo": "102302002113" }, { "bankDeposit": "中国工商银行股份有限公司无锡太湖国家旅游度假区支行", "unionBankNo":
   * "102302002121" }, { "bankDeposit": "中国工商银行股份有限公司无锡湖滨苑分理处", "unionBankNo": "102302002130" }, {
   * "bankDeposit": "中国工商银行股份有限公司无锡长街支行", "unionBankNo": "102302002148" }, { "bankDeposit":
   * "中国工商银行股份有限公司无锡会龙桥支行", "unionBankNo": "102302002156" }, { "bankDeposit": "中国工商银行股份有限公司无锡胡埭支行",
   * "unionBankNo": "102302002172" }, { "bankDeposit": "中国工商银行股份有限公司无锡梁青支行", "unionBankNo":
   * "102302002201" }, { "bankDeposit": "中国工商银行股份有限公司无锡锡沪路支行", "unionBankNo": "102302002228" }, {
   * "bankDeposit": "中国工商银行股份有限公司无锡清扬支行", "unionBankNo": "102302002252" }, { "bankDeposit":
   * "中国工商银行股份有限公司无锡迎溪支行", "unionBankNo": "102302002316" }, { "bankDeposit": "中国工商银行股份有限公司无锡惠钱路分理处",
   * "unionBankNo": "102302002324" }, { "bankDeposit": "中国工商银行股份有限公司无锡蠡园开发区支行", "unionBankNo":
   * "102302002332" }, { "bankDeposit": "中国工商银行股份有限公司无锡硕放支行", "unionBankNo": "102302002349" }, {
   * "bankDeposit": "中国工商银行盐城通榆中路支行", "unionBankNo": "102311066155" }, { "bankDeposit":
   * "中国工商银行盐城盐马路支行", "unionBankNo": "102311066171" }, { "bankDeposit": "中国工商银行盐城盐都支行",
   * "unionBankNo": "102311066180" }, { "bankDeposit": "中国工商银行盐城亭湖支行", "unionBankNo": "102311066219"
   * }, { "bankDeposit": "中国工商银行盐城青年路支行", "unionBankNo": "102311066227" }, { "bankDeposit":
   * "中国工商银行盐城南元支行", "unionBankNo": "102311066235" } ] }
   * 
   * @param keyword
   * @param max
   * @param p
  ...
  public String queryBanks(String keyword, int max, int p) {
    ...
    if (keyword == null || "".equals(keyword)) {
      response = StringUtils.parseObjectToJSONString(new Resp(date, false,
          Constant.ILLEGAL_ARGUMENT, "请输入关键字"));
      logger.info(response);
      return response;
    }
